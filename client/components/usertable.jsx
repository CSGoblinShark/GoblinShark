...
export default function UserTable() {
    const { data, error } = useSWR("http://127.0.0.1:8090/api/collections/users/records", fetcher)
    ...
    function handleShowVerification() {
        axios.post('http://127.0.0.1:8090/api/collections/verification/records', {}).then(res => {
            { !verificationCode && setVerificationCode(res.data.id) }
        })
                                    ...
                                    {sortedUsers.map((user) => (
                                        <tr key={user.email}>
                                            ...
                                            <td className="whitespace-nowrap px-3 py-4 text-sm text-gray-500">
                                                <div className="text-gray-900">{user.cohortLocation}</div>
                                                <div className="text-gray-500">{user.cohortNum}</div>
                                            </td>
                                            ...
                                            <td className="whitespace-nowrap px-3 py-4 text-sm text-gray-500">
                                                <a href={user.linkedin} className="text-indigo-600 hover:text-indigo-900">
                                                    LinkedIn<span className="sr-only">, {user.lastName}</span>
                                                </a>
                                            </td>
                                        </tr>
                                    ))}
